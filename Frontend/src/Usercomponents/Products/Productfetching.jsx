...
export default function Productfetching({ darkMode = false, categoryId }) {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const res = await axios.get("http://localhost:4000/api/v1/auth/productFetch");
      ...
      <div className={`products-container ${darkMode ? "dark" : ""}`}>
        {products.map((product) => (<Card key={product._id} className={`product-card ${darkMode ? "dark-card" : ""}`} >
          ...
          {product.images?.length > 0 && (
            <Card.Img variant="top" src={`http://localhost:4000/uploads/${encodeURIComponent(product.images[0])}`} className="product-image" />)}

...
function SignupModal({ show, handleClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      ...
      try {
        const response = await axios.post("http://localhost:4000/api/v1/auth/Loginpost", userdetails);
    ...
    try {
      const response = await axios.post("http://localhost:4000/api/v1/auth/userpost", userdetails);

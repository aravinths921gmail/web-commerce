...
export default function AdminOrders() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch all orders
        const ordersRes = await axios.get("http://13.49.230.178:4000/api/admin/orders", {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        });

        // Fetch all refunded orders
        const refundsRes = await axios.get(
          "http://13.49.230.178:4000/api/v1/orders/getRefundedOrders",
          { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
        );
  ...
  const deleteOrder = async (id) => {
    try {
      await axios.delete(`http://13.49.230.178:4000/api/admin/order/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });

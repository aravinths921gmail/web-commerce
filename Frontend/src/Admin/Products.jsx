...
export default function Products() {
    ...
    const fetchProducts = async () => {
      try {
        const res = await axios.get(
          "http://localhost:4000/api/v1/auth/productFetch",
  ...
  const addProduct = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        "http://localhost:4000/api/v1/auth/ProductCreate",
        formData,
  ...
  const deleteProduct = async (id) => {
    try {
      await axios.delete(`http://localhost:4000/api/v1/auth/deleteProduct/${id}`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
  ...
  const updateProduct = async (id) => {
    try {
      const res = await axios.put(
        `http://localhost:4000/api/v1/auth/updateProduct/${id}`,
        {
              ...
              <>
                ...
                <div style={{ display: "flex", gap: "5px", margin: "10px 0" }}>
                  {p.images && p.images.map((img, idx) => (
                    <img
                      key={idx}
                      src={`http://localhost:4000/uploads/${encodeURIComponent(img)}`}
                      alt={p.Name}
                      width="80"
